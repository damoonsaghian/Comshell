...
function storeBuffer(buffer, projectName) {
  const data = JSON.stringify(buffer.serialize({ history: false }));
...
function openProjectPane(projectName) {
  ...
  fs.readFile(storePath, { encoding: 'utf8' }, (err, data) => {
    ...
    let projectPane;
    if (serializedPane) {
      projectPane = atom.deserializers.deserialize(serializedPane);
      //activePane.constructor.deserialize(serializedPane, atom);
    } else {
