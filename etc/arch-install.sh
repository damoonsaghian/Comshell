...
printf "set superusers=\"\"" > /etc/grub.d/40_custom
printf "CLASS=\"--class gnu-linux --class gnu --class os --unrestricted\"" > /etc/grub.d/10_linux
...
# automatic time_zone:
#!/bin/sh
printf 'if [ "$2" = "connectivity-change" ] && [ -z "$VPN_IP_IFACE" ]; then
